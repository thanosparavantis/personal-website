...
export default function ProjectTag({tag, active = false}) {
  ...
  return (
    <Link to={active ? "/projects" : `/projects/tag/${tag}`}
          ...
          className={`group flex border rounded text-sm ${active ? "text-white bg-green-500 border-green-600" : "text-gray-600 bg-gray-50 border-gray-400 hover:border-gray-600"}`}>
